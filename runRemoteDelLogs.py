        ...
        with open('Info','r') as f:
            for line in f.readlines():
                if '#' not in line:
                    ...
                    execute_command = "cd /tmp/scripts && python deleteLogs.py "+dir+' '+regexs+' '+howmanydays+' '+isRemoveFile+' '+isRemoveDirs
                    stdin, stdout, stderr, exit_code = self._sshclient.ssh_exec_command("mkdir -p /tmp/scripts")
                    if exit_code:
                        print("mkdir /tmp/scripts fail!"+'\r\n'+stderr.read())
                    self._sshclient.sftp_put('deleteLogs.py', os.path.join('/tmp/scripts', "deleteLogs.py"))
                    stdin, stdout, stderr, exit_code=self._sshclient.ssh_exec_command(execute_command)
                    if exit_code:
                        print("execute python deleteLogs.py fail!"+'\r\n'+stderr.read())
                    delscript_command = "cd /tmp/scripts && rm -rf deleteLogs.py"
